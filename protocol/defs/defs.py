...
class FixedPOneDec8(InformationObj):
	...
	def update(self):
		bits = BitStream()
		bits.append(Bits(f"int:8={int(float(self.value)*10):d}"))
		self.rawdata = bits.bytes
...
class FixedPOneDec16(InformationObj):
	...
	def update(self):
		bits = BitStream()
		bits.append(Bits(f"int:16={int(float(self.value)*10):d}"))
		self.rawdata = bits.bytes
...
class FixedPTwoDec16(InformationObj):
	...
	def update(self):
		bits = BitStream()
		bits.append(Bits(f"int:16={int(float(self.value)*100):d}"))
		self.rawdata = bits.bytes
